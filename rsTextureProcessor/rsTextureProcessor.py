...
#---------------------------------------
#scrript name : rsTextureProcessor.py
#               by Meng-Han Ho
#               2017/03/02
#---------------------------------------
#   this script convert rexture to redshift format
#
#---------------------------------------
...
#This script use redshift textureprocessor cmd
...
#--------------------------------------
#2017/03/02
#   make this script based on makeTx script
#


import sys,os,glob,platform,time
import UI_rsTextureProcessor as rsTextureProcessorUI
from PySide.QtCore import   *
from PySide.QtGui import *
from subprocess import call

##test if running in Maya
try:
...
#if running in Maya is true, loading necessary packages
#maya_main_window() is just simple way to wrap UI inot MayaUI
#doIt is the entry call for this script in Maya
    ...
    def run(self, *args):
        while self.existing:
            for i in range(len(args[0][0])):
                #print args[1]
                args[1].writeToLog('JOB LOG:  converting ' + str(i + 1) + '/' + str(len(args[0][0])))
                #print args[0][0][i]
                #if args[2] == 0:
                        #print 'OIIO'
                #elif args[2]==1:
                        #print 'MAKETX'
                if args[1].runningOS != 'windows':
                    call(args[0][0][i])
                else:
                    call(args[0][0][i], shell=True)
            #args[1].writeToLog('JOB LOG: Finished')
            self.existing = False

#main class
class MainWindow(QMainWindow, rsTextureProcessorUI.Ui_MainWindow):
    #__init__
    #       contain many variable sync with UI elements
    #       so, we don't need to call UI elements each time we want to know
    #       what's user is setting up for
    def __init__(self,parent=None):
        super(MainWindow, self).__init__(parent)
        self.setupUi(self)

        #enable Maya tab if running in Maya
        self.tabWid.setTabEnabled(2, mayaModeStatus.standalong)
        self.msgSec = 3000

        #os platform
        cmdName = 'redshiftTextureProcessor'
        rsPath='/usr/redshift/bin'
        #oiioName = 'oiiotool'
        self.runningOS = 'linux'
        os_platform = platform.system()
        if 'windows' in os_platform.lower() or 'win' in os_platform.lower():
            cmdName = 'redshiftTextureProcessor.exe'
            rsPath= 'C:\ProgramData\Redshift\bin'
            #oiioName = 'oiiotool.exe'
            self.runningOS = 'windows'
        elif 'darwin' in os_platform.lower():
            self.runningOS = 'mac'
        
        #find out __file__ location, to get default maketx, oiiotool location
        cmdDir = os.path.dirname(os.path.abspath(rsPath))
        ...
        if os.path.isfile(self.cmdPath):
            ...
            self.writeToLog('redshiftTextureProcessor not existd!')

        self.fileNodeType = ['file', 'RedshiftNormalMap']
        ...
        #self.pB_selTarget.clicked.connect(self.mayaSelectNode)
        #menu
        #about
        # cmd flag
        self.ac_aboutTextureProcessor.triggered.connect(lambda: self.showCmdHelp(self,'http://docs.redshift3d.com/Default.html#I/Texture%20Processor%20Tool.html'))
        #self.ac_aboutMakeTx.triggered.connect(lambda: self.showCmdHelp(self,'https://manned.org/maketx.1'))
        ...
        self.worker.terminated.connect(self.threadTerminated)

        #show ui
        self.show()
    
    def threadStarted(self):
        self.writeToLog('job started')

    def threadFinished(self):
        if self.ac_confirm.isChecked():
            QMessageBox.information(self, 'Info', 'Job finished.')
        self.writeToLog('Job Finished')

    def threadTerminated(self):
        self.writeToLog('job terminated.')

    def threadJobQueue(self, *args):
        if not self.worker.isRunning():
            self.worker.existing = True
            self.worker.run(args[0:-1], self,args[-1])
        ...
        if genList:
            for x in genList:
                ...
                if len(baseNameExtract) > 1:
                    for i in range(len(baseNameExtract)):
                        baseName += baseNameExtract[i]
                        if i < len(baseNameExtract) - 1:
                            baseName += '.'
    ...
    def showDirBrowser(self,*arg):
        ...
        dlg.setFilter(self.browserImageFilter())
        fName = False
        if dlg.exec_():
    ...
    def writeToLog(self,inputLog):
        self.statusBar().showMessage(inputLog,self.msgSec)
        pass
    ...
    def showAbout(self):
        aboutText = 'redshift Texture Processor gui beta \r\n'
    ...
    def createBatchFile(self):
        ...
        if cmdList:
            ...
            QMessageBox.information(self,'Information','There are no files need to be converted. No file nodes found or tx files alread exist.')
    ...
    def matchTypeInHistory(self,matchType,sel):
        outList = []
        for node in sel:
            tmpList = [x for x in cmds.listHistory(node) if cmds.nodeType(x) in matchType]
            if tmpList:
                outList+=tmpList
        return outList
        
    def mayaSelectNode(self):
        try:
            if self.mayaConvertAll:
                self.returnAllTextureList()
            else:
                self.returnTextureListFromSelect()
            if self.PROCESS_NODE:
                cmds.select(self.PROCESS_NODE,r=True)
        except:
            pass
    ...
    def returnTextureListFromSelect(self):
        ...
        #fileNode = []
        #Use self.fileNodeType from __init__ instead
        #fileNodeType = ['file']
        selGrp = sel
        #print selGrp
        ...
        self.PROCESS_NODE = []
        for x in sel:
            #sEngine = cmds.listConnections(x,type='shadingEngine')
            sEngine = [i for i in cmds.listHistory(x,f=True,af=True,ac=True) if cmds.objectType(i)=='shadingEngine']
            if sEngine:
                sEngineL = []
                for i in sEngine:
                    tmp = cmds.listHistory(i,ac=True)
                    if tmp:
                        selGrp = selGrp+tmp
                #selGrp.remove(x)

        self.PROCESS_NODE = self.matchTypeInHistory(self.fileNodeType,selGrp)
        #print self.PROCESS_NODE
        self.PROCESS_NODE = list(set(self.PROCESS_NODE))
        #print self.PROCESS_NODE
        outList = []
        for x in self.PROCESS_NODE:
            try:
                if cmds.objectType(x)=='RedshiftNormalMap':
                    tF= cmds.getAttr(x+'.tex0')
                else:    
