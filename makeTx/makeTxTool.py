...
#   2016/04/17
#   version: 1.0
#   add thread worker, prevernting window freezing while converting
#   fix oiio code
#   fix baseNameExtract so files can have dot in names
#
#--------------------------------------
#   2016/04/05
#   version: beta 0.9
...
#   error handle, there are almost none
#   better workflow control
#   optimized qthread
#   use listHistory in Maya to get related file nodes, instead of recursive def
...
class MainWindow(QMainWindow, maketxUI.Ui_MainWindow):
    ...
    def __init__(self,parent=None):
        ...
        if os.path.isfile(self.oiioCmd):
            ...
            self.writeToLog('no default oiio command existd!')
            self.oiioCmd = ''
            pass

        #
        self.fileNodeType = ['file']
        ...
        #about
        # cmd flag
        self.ac_aboutOIIO.triggered.connect(lambda: self.showCmdHelp(self,'https://manned.org/oiiotool.1'))
    ...
    def oiioGo(self):
        ...
        if oiioCmdList:
            #print oiioCmdList
            self.threadJobQueue(oiioCmdList,0)
            ...
            if sourceF and self.oiioFlag:
                for i in sourceF:
                    ...
                    if len(baseNameExtract)>1:
                        for x in range(len(baseNameExtract)):
                            baseName += baseNameExtract[x]
                            if x < len(baseNameExtract)-1:
                                baseName += '.'
